...
class CrossValidator:
    def __init__(
        self,
        arc_metadata: List[Dict[str, Any]],
        arc_tag_files: List[Path],
        day_files: List[Path],
    ):
        self.arc_metadata = arc_metadata
        self.arc_tag_data = {
            p.stem.replace("_tags", ""): load_yaml(p) for p in arc_tag_files
        }
    ...
    def _check_arc_tag_consistency(self) -> List[str]:
        ...
        for arc_id in metadata_arcs - tag_arcs:
            errors.append(
                f"❌ Arc ID '{arc_id}' in arc_metadata.yaml not found in arc_tags/*.yaml"
            )
        for arc_id in tag_arcs - metadata_arcs:
            errors.append(
                f"❌ Arc ID '{arc_id}' in arc_tags/*.yaml not found in arc_metadata.yaml"
            )
        return errors
    ...
    def _check_day_arc_consistency(self) -> List[str]:
        ...
        for fname, data in self.day_data.items():
            ...
            if arc_id and arc_id not in metadata_arcs:
                errors.append(
                    f"❌ Arc ID '{arc_id}' in {fname} not found in arc_metadata.yaml"
                )
        return errors
        ...
        for arc_id, arc_meta in metadata_arcs.items():
            ...
            if tag_data:
                if tag_data.get("arc_title") != meta_title:
                    errors.append(
                        f"❌ arc_title mismatch for {arc_id} in arc_tags/*.yaml"
                    )
    ...
    def validate(self) -> List[str]:
        ...
        for num, data in self.day_data.items():
            if num != data.get("master_day_number"):
                errors.append(
                    f"❌ day_{num:04d}.yaml has mismatched master_day_number: {data.get('master_day_number')}"
                )
            ...
            if arc_day in arc_day_seen[arc_id]:
                errors.append(
                    f"❌ Duplicate arc_day_number {arc_day} in arc '{arc_id}'"
                )
    ...
    def run(self) -> None:
        ...
        for tag_file in arc_tag_files:
            ...
            if data:
                errors.extend(
                    self.schema_validator.validate(
                        data, self.schema_validator.arc_tag_schema, tag_file.name
                    )
                )
        ...
        for arc in arc_metadata:
            errors.extend(
                self.tag_validator.validate_tags(
                    arc.get("tags", []), f"arc_metadata.yaml [{arc.get('arc_id')}]"
                )
            )
        for tag_file in arc_tag_files:
            data = load_yaml(tag_file)
            if data:
                for category in TAG_CATEGORIES:
                    errors.extend(
                        self.tag_validator.validate_tags(
                            data.get("tags", {}).get(category) or [], tag_file.name
                        )
                    )
        for day_file in day_files:
            data = load_yaml(day_file)
